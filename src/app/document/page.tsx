...
import BaseDynamicForm from "@/components/common/BaseDynamicForm";
...
export default function Services() {
  ...
  const fetchData = useCallback(
    async (page: number, pageSize: number) => {
      ...
      try {
        ...
        if (result.code === 0) {
          if (result.content) {
            let responseData = result.content.items || [];
            setData(responseData);
            setTotalRecords(result.content.count); // Set total records for pagination
  ...
  }, [page, pageSize]); // Chỉ chạy khi page hoặc pageSize thay đổi

  const columnDefs = [
    { headerName: "ID", field: "id" },
    { headerName: "Trạng thái", field: "statusDesc" },
    { headerName: "Chi tiết trạng thái", field: "statusDetail" },
    { headerName: "Số hợp đồng", field: "contract.number" },
    { headerName: "Tên gói dịch vụ", field: "contract.servicePack" },
    { headerName: "Thời hạn (tháng)", field: "contract.validity" },
    { headerName: "Người dùng", field: "identity.name" },
    { headerName: "Email", field: "identity.email" },
    { headerName: "SĐT", field: "identity.phone" },
    { headerName: "Ngày tạo", field: "orderInfo.createdDate" },
    {
      headerName: "Tên hồ sơ chứng chỉ",
      field: "orderInfo.certificateProfileName",
    },
    { headerName: "Tên dịch vụ", field: "certProfile.serviceType" },
    { headerName: "Loại chữ ký", field: "signType" },
    { headerName: "Hành động", field: "actions" },
  ];
